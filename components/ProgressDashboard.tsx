...
import { fetchWorkoutHistory, fetchWorkoutStreak } from '../services/database';
...
export const ProgressDashboard: React.FC<ProgressDashboardProps> = ({ profile, lang }) => {
    ...
    useEffect(() => {
        const loadData = async () => {
            ...
            const [history, streak] = await Promise.all([
                fetchWorkoutHistory(profile.id, 30),
    ...
    const chartData = workoutHistory
        ...
        .reverse()
        .map(w => ({
            date: new Date(w.completed_at).toLocaleDateString('pt-BR', { day: '2-digit', month: '2-digit' }),
    ...
    if (loading) {
        return (
            <div className="min-h-screen bg-slate-900 text-white p-6 flex items-center justify-center">
                <p className="text-xl font-black">Carregando...</p>
            </div>
        );
    }
    ...
    return (
        <div className="min-h-screen bg-slate-900 text-white pb-24">
            {/* Header */}
            <div className="bg-gradient-to-br from-indigo-600 to-indigo-800 p-8 mb-6">
                <h1 className="text-3xl font-black italic uppercase tracking-tighter mb-2">Seu Progresso</h1>
                <p className="text-sm opacity-60">Evolução e conquistas</p>
            </div>
            ...
            <div className="max-w-md mx-auto px-6 space-y-6">
                ...
                <div className="bg-slate-800 p-6 rounded-[35px] border border-slate-700/50">
                    <h3 className="text-[10px] font-black uppercase opacity-40 mb-4 tracking-widest">Estatísticas Gerais</h3>
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <p className="text-[9px] font-black uppercase opacity-40 mb-1">Volume Total</p>
                        ...
                        <div>
                            <p className="text-[9px] font-black uppercase opacity-40 mb-1">Séries</p>
                            <p className="text-2xl font-black">{totalSets}</p>
                        </div>
                        <div>
                            <p className="text-[9px] font-black uppercase opacity-40 mb-1">Duração Média</p>
                            <p className="text-2xl font-black">{avgDuration}min</p>
                        </div>
                        <div>
                            <p className="text-[9px] font-black uppercase opacity-40 mb-1">Este Mês</p>
                ...
                {chartData.length > 0 && (
                    <div className="bg-slate-800 p-6 rounded-[35px] border border-slate-700/50">
                        ...
                        <p className="text-[9px] text-center opacity-40 mt-2">Volume em toneladas (t)</p>
                    </div>
                )}

                {/* Recent Workouts */}
                <div className="bg-slate-800 p-6 rounded-[35px] border border-slate-700/50">
                    <h3 className="text-[10px] font-black uppercase opacity-40 mb-4 tracking-widest">Treinos Recentes</h3>
                ...
                {workoutHistory.length === 0 && (
                    <div className="bg-slate-800 p-10 rounded-[40px] text-center border border-slate-700/50">
                        ...
                        <p className="font-black mb-2">Nenhum treino ainda</p>
                        <p className="text-sm opacity-60">Complete seu primeiro treino para ver estatísticas!</p>
                    </div>
                )}
            </div>
        </div>
