...
export const CompletedWorkoutSummary: React.FC<CompletedWorkoutSummaryProps> = ({ sessionData, lang, onFinish }) => {
    ...
    useEffect(() => {
        ...
        createConfetti(); // Immediate burst

        return () => clearInterval(interval);
    }, []);

    return (
        <div className="min-h-screen bg-slate-900 text-white p-6 space-y-6 pb-24 relative overflow-hidden">
            {/* Confetti Setup */}
            <style>{`
                @keyframes fall {
            ...
            {/* Celebration Header */}
            <div className="text-center space-y-6 pt-10 pb-6 relative z-10">
                ...
                <div>
                    <h1 className="text-4xl font-black italic uppercase tracking-tighter mb-2 animate-in zoom-in duration-500">
                        Treino Completo!
                    </h1>
                    <p className="text-sm opacity-60 uppercase font-black tracking-widest animate-in slide-in-from-bottom duration-700">
                        Excelente trabalho! 💪
                    </p>
                </div>
            </div>
            ...
            <div className="bg-gradient-to-br from-indigo-600 to-indigo-800 p-8 rounded-[40px] shadow-2xl">
                ...
                <div className="grid grid-cols-2 gap-4">
                    ...
                    {/* Weight Removed per user request */}
                    <StatCard
                        icon={Repeat}
                        label="Séries"
