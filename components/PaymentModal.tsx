...
export const PaymentModal: React.FC<PaymentModalProps> = ({ planName, price, onClose, onSuccess, lang }) => {
        ...
        return (
            <div className="fixed inset-0 z-50 bg-black/90 backdrop-blur-sm flex items-center justify-center p-6 animate-in fade-in zoom-in duration-300">
                <div className="bg-slate-800 p-8 rounded-[40px] border border-green-500/50 text-center max-w-sm w-full shadow-2xl relative overflow-hidden">
                    ...
                    <h2 className="text-2xl font-black italic uppercase tracking-tighter mb-2 text-white">{t.payment.successTitle}</h2>
                    <p className="text-white/60 text-sm mb-6">{t.payment.successSubtitle.replace('{plan}', planName)}</p>
            ...
            <div className="bg-slate-900 w-full max-w-md rounded-t-[40px] sm:rounded-[40px] border-t sm:border border-slate-700/50 shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
                ...
                <div className="p-6 bg-slate-800 border-b border-slate-700/50 flex justify-between items-center">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h3 className="font-black italic uppercase text-lg text-white">{t.payment.secureCheckout}</h3>
                            <p className="text-[10px] text-slate-400 uppercase tracking-widest flex items-center gap-1"><Lock size={10} /> {t.payment.encryption}</p>
                        </div>
                    </div>
                ...
                <div className="p-8 text-center bg-indigo-600/10 border-b border-indigo-500/10">
                    <p className="text-xs uppercase font-black text-indigo-300 tracking-widest mb-1">{t.payment.totalToPay}</p>
                    <h1 className="text-4xl font-black italic text-white tracking-tighter">{price}</h1>
                    <p className="text-[10px] opacity-60 mt-2 uppercase">{planName}</p>
                </div>
                ...
                <form onSubmit={handlePay} className="p-6 space-y-4 overflow-y-auto">
                    ...
                    <div className="space-y-1">
                        <label className="text-[10px] font-black uppercase text-slate-400 ml-2">{t.payment.cardNumber}</label>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div className="space-y-1">
                            <label className="text-[10px] font-black uppercase text-slate-400 ml-2">{t.payment.expiry}</label>
                            <input required className="w-full bg-slate-800 border border-slate-700 p-4 rounded-2xl text-white outline-none focus:border-indigo-500 transition-all font-mono text-center" placeholder="MM/YY" />
                        </div>
                        <div className="space-y-1">
                            <label className="text-[10px] font-black uppercase text-slate-400 ml-2">{t.payment.cvv}</label>
                    ...
                    <button disabled={processing} className="w-full bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 disabled:cursor-not-allowed p-5 rounded-2xl font-black uppercase text-white shadow-xl flex items-center justify-center gap-2 mt-4 transition-all active:scale-95">
                        {processing ? (
                            ...
                            <>{t.payment.payButton} {price}</>
                        )}
                    </button>
                    <p className="text-center text-[9px] text-slate-500 uppercase mt-4">{t.payment.poweredBy}</p>
                </form>
            </div>
        </div>
    );
