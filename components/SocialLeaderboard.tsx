...
export const SocialLeaderboard = () => {
    ...
    const fetchData = async () => {
        try {
            ...
            // Fallback manual query or static data
            const { data: profiles } = await supabase.from('user_profiles').select('id, name, level').limit(10);

            // Static fallback if DB is empty
            const staticProfiles = [
                { id: 'm1', name: 'Rafael Silva', level: 'Advanced' },
                { id: 'm2', name: 'Ana Costa', level: 'Intermediate' },
                { id: 'm3', name: 'Carlos Gym', level: 'Pro' },
                { id: 'm4', name: 'Julia Fit', level: 'Beginner' },
                { id: 'm5', name: 'Pedro Strong', level: 'Advanced' }
            ];

            const profilesToUse = (profiles && profiles.length > 0) ? profiles : staticProfiles;

            // Mock stats for demo
            const mockLeaders = profilesToUse.map(p => ({
                ...p,
                workouts: Math.floor(Math.random() * 20) + 5, // Guarantee at least 5 workouts
