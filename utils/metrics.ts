...
export const calculateMacros = (profile: UserProfile): { protein: number; fats: number; carbs: number } => {
  ...
  // Fats: 0.8g per kg (Healthy minimum)
  const fatGrams = Math.round(profile.weight * 0.8);
  const fatCals = fatGrams * 9;

  // Carbs: Remainder
  const remainingCals = target - proteinCals - fatCals;
  const carbGrams = Math.max(0, Math.round(remainingCals / 4));

  return { protein: proteinGrams, fats: fatGrams, carbs: carbGrams };
};
