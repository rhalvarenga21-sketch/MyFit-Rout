  ...
  return (
    <div className="min-h-screen bg-slate-900 text-slate-50 font-sans pb-24 selection:bg-indigo-500/30">
      ...
      <main className="max-w-md mx-auto p-5">
              ...
              <div className="space-y-6">
                ...
                <button onClick={() => setView('progress')} className="w-full bg-gradient-to-r from-indigo-600 to-purple-600 p-6 rounded-[35px] flex items-center justify-between group hover:scale-[1.02] transition-all shadow-xl">
                  <div className="flex items-center gap-4">
                    ...
                    <div className="text-left">
                      <p className="font-black uppercase text-sm tracking-widest">Meu Progresso</p>
                      <p className="text-[10px] opacity-60 uppercase">Estatísticas e conquistas</p>
                    </div>
                  </div>
        ...
        {view === 'workout_active' && selectedWorkout && currentUser && (
          <ActiveWorkout
            workout={selectedWorkout as PresetWorkout}
            lang={lang}
            ...
            onComplete={async (sessionData) => {
              ...
              setIsSyncing(true);
              await completeWorkoutSession(sessionData);
              setIsSyncing(false);

              // Update completed days
              const today = new Date().toISOString().split('T')[0];
              if (profile && !profile.completedDays.includes(today)) {
                await saveProfile({ ...profile, completedDays: [...profile.completedDays, today] });
              }
