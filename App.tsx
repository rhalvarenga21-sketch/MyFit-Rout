...
import { 
  ...
  // Added missing Target and Timer icons
  Target, Timer
} from 'lucide-react';
import { 
  Language, UserProfile, DayPlan, UserRole, PresetWorkout, Gender, ActivityLevel, FitnessGoal, ExperienceLevel
} from './types';
...
const App: React.FC = () => {
  const [profile, setProfile] = useState<UserProfile | null>(() => {
    const saved = localStorage.getItem('myfitrout_v5_profile');
    return saved ? JSON.parse(saved) : null;
  });
  const [lang, setLang] = useState<Language>(profile?.language || Language.PT);
  const [view, setView] = useState<'home' | 'planning' | 'vital' | 'me' | 'workout_summary' | 'workout_active' | 'b2b'>('home');
  ...
  const saveProfile = (newP: UserProfile) => {
    setProfile(newP);
    localStorage.setItem('myfitrout_v5_profile', JSON.stringify(newP));
  };
  ...
  const metrics = useMemo(() => {
    if (!profile) return { bmr: 0, tdee: 0, water: 0, target: 0 };
    const { weight, height, age, gender, activityLevel } = profile;
    const bmr = gender === Gender.MALE ? (10 * weight) + (6.25 * height) - (5 * age) + 5 : (10 * weight) + (6.25 * height) - (5 * age) - 161;
    const mults: any = { SEDENTARY: 1.2, LIGHT: 1.375, MODERATE: 1.55, HEAVY: 1.725, ATHLETE: 1.9 };
    const tdee = Math.round(bmr * mults[activityLevel]);
    return { bmr: Math.round(bmr), tdee, water: Math.round(weight * 35), target: tdee + 300 };
  }, [profile]);
      ...
      <main className="max-w-md mx-auto p-5">
        {view === 'home' && (
          <div className="space-y-6 animate-in fade-in">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <MetricBox icon={Utensils} label={t.profile.targetKcal} value={`${metrics.target}kcal`} color="text-orange-400" />
            </div>

            <section className="bg-slate-800 p-6 rounded-[35px] border border-slate-700/50">
               <h3 className="font-black text-sm uppercase tracking-widest mb-4 flex items-center gap-2 text-slate-400"><Clock size={16}/> {t.mon === 'Segunda' ? 'HOJE' : 'TODAY'}</h3>
               {profile.customSchedule[DAYS[new Date().getDay() - 1 === -1 ? 6 : new Date().getDay() - 1]]?.type === 'WORKOUT' ? (
                 <button onClick={() => {
                    const id = profile.customSchedule[DAYS[new Date().getDay() - 1 === -1 ? 6 : new Date().getDay() - 1]].presetWorkoutId;
                    setSelectedWorkout(PRESET_WORKOUTS.find(w => w.id === id)!);
                    ...
                    <span className="font-black">Start {PRESET_WORKOUTS.find(w => w.id === profile.customSchedule[DAYS[new Date().getDay() - 1 === -1 ? 6 : new Date().getDay() - 1]].presetWorkoutId)?.title}</span>
          ...
          <div className="space-y-6 animate-in slide-in-from-bottom-6">
            <div className="flex justify-between items-end">
              <div>
                ...
                <p className="text-xs font-bold opacity-40">{t.plan.subtitle}</p>
              </div>
              <button className="bg-indigo-500/10 text-indigo-400 p-2 rounded-xl border border-indigo-500/20"><Plus size={18}/></button>
            </div>
            
            <div className="space-y-3">
              {DAYS.map(day => (
                <div key={day} className="bg-slate-800 p-5 rounded-3xl border border-slate-700/50 flex justify-between items-center group hover:border-indigo-500/30 transition-all">
                   <div className="flex items-center gap-4">
                      ...
                      <div>
                         <p className="font-black text-sm">{profile.customSchedule[day]?.type === 'REST' ? t.plan.rest : PRESET_WORKOUTS.find(w => w.id === profile.customSchedule[day].presetWorkoutId)?.title}</p>
                         <p className="text-[10px] font-black uppercase opacity-30">{profile.customSchedule[day]?.type === 'WORKOUT' ? `${PRESET_WORKOUTS.find(w => w.id === profile.customSchedule[day].presetWorkoutId)?.duration}min • ${PRESET_WORKOUTS.find(w => w.id === profile.customSchedule[day].presetWorkoutId)?.category}` : 'Active Rest'}</p>
                      </div>
                   </div>
            ...
            {editingDay && (
              <div className="fixed inset-0 z-50 bg-slate-900/90 backdrop-blur-xl flex items-end justify-center p-4">
                 <div className="bg-slate-800 w-full max-w-md p-8 rounded-[40px] border border-slate-700 space-y-6 animate-in slide-in-from-bottom duration-300">
                    <div className="flex justify-between items-center">
                       <h3 className="text-xl font-black uppercase">{t.plan.edit}: {t[editingDay]}</h3>
                    ...
                    <div className="max-h-[50vh] overflow-y-auto space-y-2 pr-2 custom-scroll">
                       ...
                       {PRESET_WORKOUTS.map(p => (
                         <button key={p.id} onClick={() => {
                            const newSched = { ...profile.customSchedule, [editingDay]: { type: 'WORKOUT', presetWorkoutId: p.id } as DayPlan };
                            saveProfile({ ...profile, customSchedule: newSched });
                            setEditingDay(null);
                         }} className="w-full bg-slate-700/30 p-5 rounded-2xl text-left font-black border-2 border-transparent hover:border-indigo-500 transition-all flex items-center justify-between">
                            <div>
                              <p>{p.title}</p>
                              <p className="text-[10px] font-black opacity-40 uppercase tracking-widest">{p.category} • {p.duration}m</p>
                            </div>
                            <Play size={14} opacity={0.3}/>
                         </button>
                       ))}
                    </div>
                 </div>
             ...
             <section className="bg-slate-800 p-6 rounded-[35px] border border-slate-700/50 flex flex-col h-[65vh] shadow-xl">
                ...
                <div className="flex-1 overflow-y-auto space-y-4 mb-4 pr-2 custom-scroll text-sm font-medium">
                   <div className="bg-indigo-500/5 p-5 rounded-3xl border-l-4 border-indigo-500 text-indigo-100/80 leading-relaxed">
                      {lang === 'PT' ? `Olá ${profile.name}. Como posso te ajudar a treinar com mais segurança hoje?` : `Hello ${profile.name}. How can I help you train safer today?`}
                   </div>
                   ...
                   {aiLoading && <div className="animate-pulse text-indigo-400 font-black text-[10px] uppercase flex items-center gap-2 px-2"><Activity size={12}/> Analyzing technical bio-data...</div>}
                </div>
                <div className="flex gap-2">
                   <input 
                    placeholder={t.askAi}
        ...
        {view === 'me' && (
          <div className="space-y-6 animate-in slide-in-from-right-6">
            ...
            <div className="grid grid-cols-2 gap-4">
               ...
               <MetricCard icon={Activity} label={t.profile.bmr} value={`${metrics.bmr}kcal`} />
               <MetricCard icon={Zap} label={t.profile.tdee} value={`${metrics.tdee}kcal`} />
        ...
        {view === 'workout_summary' && selectedWorkout && (
          <div className="space-y-4">
            <button onClick={() => setView('home')} className="p-3 bg-slate-800 rounded-2xl border border-slate-700 flex items-center gap-2 font-black text-xs opacity-60"><ChevronLeft size={16}/> BACK</button>
            <WorkoutSummary 
        ...
        {view === 'workout_active' && selectedWorkout && (
          <div className="space-y-8 animate-in slide-in-from-right-6 pb-24">
             <header className="flex justify-between items-center">
                <h3 className="font-black text-xl italic text-indigo-400">{selectedWorkout.title}</h3>
                <Timer className="text-indigo-500 animate-pulse" />
             </header>
             <div className="space-y-8">
                {selectedWorkout.mainBlockIds.map(id => {
                    ...
                    <div key={id} className="bg-slate-800 p-8 rounded-[45px] border border-slate-700/50 space-y-6 shadow-xl">
                       <div className="flex justify-between items-start">
                          <div className="space-y-1">
                            <h4 className="text-2xl font-black leading-tight">{ex.name}</h4>
                            <div className="flex gap-2">
                          ...
                          <div className="bg-indigo-600 px-4 py-2 rounded-2xl font-black text-lg shadow-lg">{ex.sets} x {ex.reps}</div>
                       </div>
        ...
        {view === 'b2b' && (
          <div className="space-y-6 animate-in slide-in-from-bottom">
            <header className="flex justify-between items-center">
              <h2 className="text-2xl font-black">{t.b2b.dashboard}</h2>
              <button onClick={() => setView('home')} className="p-2 hover:bg-slate-800 rounded-full"><X/></button>
            </header>
            
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-slate-800 p-6 rounded-[35px] border border-slate-700 space-y-1">
                 <p className="text-[10px] font-black uppercase opacity-40">{t.b2b.activeUsers}</p>
                 <p className="text-3xl font-black text-indigo-400">1.2k</p>
                 <div className="flex items-center gap-1 text-green-400 text-[10px] font-black">
                   <TrendingUp size={12}/> +12%
                 </div>
              </div>
              <div className="bg-slate-800 p-6 rounded-[35px] border border-slate-700 space-y-1">
                 <p className="text-[10px] font-black uppercase opacity-40">{t.b2b.retention}</p>
                 <p className="text-3xl font-black text-violet-400">94%</p>
                 <div className="flex items-center gap-1 text-green-400 text-[10px] font-black">
            ...
            <section className="bg-slate-800 p-6 rounded-[35px] border border-slate-700 space-y-4">
               <h3 className="font-black text-sm uppercase tracking-widest opacity-40">{t.b2b.engagement} Metrics</h3>
               <div className="space-y-3">
                 {[
                   { label: "Longevity Program", val: "78%" },
                   { label: "Strength Foundations", val: "62%" },
                   { label: "Recovery Sessions", val: "45%" }
                 ].map((p, i) => (
                   <div key={i} className="space-y-2">
                     <div className="flex justify-between text-xs font-black"><span>{p.label}</span><span>{p.val}</span></div>
                     <div className="h-2 bg-slate-900 rounded-full overflow-hidden"><div className="h-full bg-indigo-500" style={{width: p.val}}></div></div>
                   </div>
                 ))}
               </div>
            </section>
       ...
       <div className="flex gap-4 max-w-md mx-auto w-full mt-10">
          ...
          <button onClick={() => {
            if (step < 4) setStep(step + 1);
              ...
              const defaultSchedule: Record<string, DayPlan> = {};
              DAYS.forEach(d => defaultSchedule[d] = { type: 'REST' } as DayPlan);
              onComplete({ ...fd, language: lang, role: UserRole.MEMBER, trialStartDate: new Date().toISOString(), completedDays: [], feedbackHistory: [], customSchedule: defaultSchedule });
            }
